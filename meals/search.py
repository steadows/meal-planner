...
def find(request: str) -> tuple[RecipeOption, ...]:
    ...
    Raises ValueError for a blank or oversized request (before any Claude run), and
    ClaudeRunnerError if the run fails or returns more than MAX_OPTIONS options.
    """
    text = request.strip()
    if not text:
    ...
    return claude_runner.run(prompt, schema=_FindResult).options
